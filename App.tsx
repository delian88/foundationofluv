      ...
      <nav className="fixed w-full z-50 glass border-b border-[#332d2b]/10">
        <div className="max-w-7xl mx-auto px-4 h-20 md:h-28 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-10">
            {NAVIGATION.map((item) => (
              <a key={item.name} href={item.href} className="text-[10px] font-cinzel font-bold uppercase tracking-[0.3em] text-[#332d2b] hover:text-[#9c1c22] transition-all relative group">
                {item.name}<span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#9c1c22] transition-all group-hover:w-full" />
              </a>
            ))}
            <motion.button whileHover={{ scale: 1.05 }} className="bg-[#9c1c22] text-white px-8 py-3 rounded-full text-[10px] font-cinzel font-black tracking-[0.2em] uppercase">DONATE</motion.button>
          </div>
          ...
          {isMenuOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="md:hidden glass border-b border-[#332d2b]/10 overflow-hidden shadow-2xl">
              <div className="px-6 py-10 space-y-6">
                {NAVIGATION.map((item) => <a key={item.name} href={item.href} className="block text-xl font-cinzel font-bold tracking-widest text-[#332d2b]" onClick={() => setIsMenuOpen(false)}>{item.name}</a>)}
        ...
        <div className="max-w-7xl mx-auto px-4 text-center flex flex-col items-center relative z-10">
          ...
          <div className="mt-12">
            ...
            <div className="flex flex-col md:flex-row gap-6 justify-center mt-12">
              ...
              <a href="#about" className="px-12 py-6 glass-card rounded-full font-cinzel font-bold text-xl border border-[#9c1c22]/15 flex items-center justify-center gap-3">Explore Our Story <ArrowRight /></a>
            </div>
          </div>
        </div>
      </header>
                ...
                <div>
                  ...
                  <div className="space-y-10">
                    {STRATEGIC_PHASES[activeRoadmap].goals.map((goal, idx) => (
                      <div key={idx} className="flex gap-7 items-start"><div className="w-2 h-2 bg-[#9c1c22] rounded-full mt-2" /><p className="text-xl text-[#fdfaf6]/80 font-serif italic">{goal}</p></div>
                    ))}
                  </div>
                </div>
                <div className="bg-[#fdfaf6] text-[#332d2b] rounded-[3.5rem] p-14 shadow-2xl">
                  ...
                  <div className="space-y-12">
                    {STRATEGIC_PHASES[activeRoadmap].outputs.map((output, idx) => (
                      <div key={idx} className="flex gap-8 items-center"><div className="text-7xl font-serif font-black italic text-[#9c1c22]/10">{idx + 1}</div><p className="text-2xl font-serif font-black leading-tight">{output}</p></div>
                    ))}
                  </div>
                </div>
              </div>
